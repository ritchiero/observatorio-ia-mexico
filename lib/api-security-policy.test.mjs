...
test('toda mutación HTTP está autorizada o declarada pública de forma explícita', async () => {
  const publicMutationAllowlist = new Set([
    ...
    // Consulta pública de la hemeroteca (mismo perfil que grafo/buscar):
    // valida longitud de entrada y su costo va contra la llave del servidor.
    'app/api/hemeroteca/buscar/route.ts#POST',
