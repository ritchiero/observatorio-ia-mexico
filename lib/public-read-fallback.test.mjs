...
test('activa el fallback sólo en desarrollo/preview y sin credenciales', () => {
  assert.equal(shouldUsePublicReadFallback({ NODE_ENV: 'development' }), true);
  assert.equal(
    shouldUsePublicReadFallback({ NODE_ENV: 'production', VERCEL_ENV: 'preview' }),
    true,
  );
  assert.equal(shouldUsePublicReadFallback({ NODE_ENV: 'production' }), false);
  assert.equal(
    shouldUsePublicReadFallback({ NODE_ENV: 'production', VERCEL_ENV: 'production' }),
    false,
  );
  assert.equal(
