...
test('errores por ficha impiden reportar una corrida completa aunque success sea true', () => {
  const c = evaluarCorrida([
    ok('monitoreo', { success: true, actualizacionesDetectadas: 2, errores: ['fuente no disponible'] }),
    ok('deteccion', { success: true, anunciosEncontrados: 1, errores: [] }),
  ]);
  assert.equal(c.ok, false);
  assert.deepEqual(c.fallidos, ['monitoreo']);
  assert.equal(c.hallazgosTotales, 3, 'no se descartan los hallazgos de la parte exitosa');
  assert.match(c.resumen, /revisión incompleta/);
});

test('cero hallazgos con errores no se confunde con una revisión sana sin novedad', () => {
  const e = evaluarAgente(ok('monitoreo', { success: true, actualizacionesDetectadas: 0, errores: ['sin saldo'] }));
  assert.equal(e.estado, 'fallo');
  assert.equal(e.hallazgos, 0);
});

test('success debe ser booleano; cadenas y números quedan indeterminados', () => {
  for (const success of ['false', 'true', 0, 1, null]) {
    assert.equal(evaluarAgente(ok('monitoreo', { success, actualizacionesDetectadas: 0 })).estado, 'indeterminado');
  }
});
