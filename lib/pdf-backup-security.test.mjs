...
test('un respaldo real exige un bucket privado distinto al bucket público', () => {
  const publicBucket = 'observatorio-ia-mexico.firebasestorage.app';

  assert.equal(
    requirePrivateBackupBucketName('observatorio-ia-private-backups', publicBucket),
    'observatorio-ia-private-backups',
  );

  for (const [value, code] of [
    [undefined, 'private_bucket_required'],
    ['', 'private_bucket_required'],
    ['gs://observatorio-ia-private-backups', 'invalid_private_bucket'],
    [publicBucket, 'public_bucket_forbidden'],
  ]) {
    assert.throws(
      () => requirePrivateBackupBucketName(value, publicBucket),
      (error) => error instanceof PdfBackupValidationError && error.code === code,
    );
  }
});
...
test('la conexión queda fijada a la IP validada conservando Host y SNI', () => {
  const options = buildPinnedHttpsRequestOptions(
    new URL('https://docs.gob.mx/ruta/a.pdf?download=1'),
    '8.8.8.8',
  );

  assert.equal(options.hostname, '8.8.8.8');
  assert.equal(options.servername, 'docs.gob.mx');
  assert.equal(options.path, '/ruta/a.pdf?download=1');
  assert.equal(options.agent, false);
  assert.equal(options.rejectUnauthorized, true);
  assert.equal(options.headers.Host, 'docs.gob.mx');
});
