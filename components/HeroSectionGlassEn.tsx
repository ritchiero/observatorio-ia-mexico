...
export default function HeroSectionGlassEn({ stats, legStats, casosStats, loading, loadingLeg, loadingCasos }: HeroGlassProps) {
  const [mounted, setMounted] = useState(false);
  const [scrolled, setScrolled] = useState(false);
  const [menuOpen, setMenuOpen] = useState(false);
  const pathname = usePathname();
  const navT = dict('en').nav;
  const homeT = dict('en').home;
  ...
  const handleSubscribe = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/suscripciones', {
        ...
        body: JSON.stringify({ ...formData, origen: pathname || '/en' }),
      });
      ...
      setFormStatus('error');
      setFormMessage(error instanceof Error ? error.message : 'Subscription failed');
    }
  };
  ...
  return (
    <div className="relative w-full min-h-screen overflow-hidden" style={{ background: T.void, color: T.text }}>
      ...
      <header className={`fixed top-0 inset-x-0 z-40 flex items-center justify-between gap-4 mx-3 md:mx-8 rounded-[100px] transition-all duration-300 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-3'} ${scrolled ? 'mt-2 px-4 md:px-7 py-2.5' : 'mt-4 md:mt-5 px-5 md:px-8 py-3.5 md:py-4'}`}
        ...
        <Link href="/en" onClick={() => setMenuOpen(false)} className="flex items-center gap-3 shrink-0 group">
        ...
        <div className="flex items-center gap-2.5 shrink-0">
          {/* Language switcher: plain <a> (no prefetch) — ?hl= sets the cookie in proxy.ts */}
          <a href="/?hl=es" title="Ver en español" className="flex items-center justify-center font-mono font-semibold transition-colors hover:text-white"
      ...
      {menuOpen && (
        <div className="md:hidden fixed inset-x-3 z-40 rounded-3xl p-3 flex flex-col gap-1" style={{
          ...
          <a href="/?hl=es" className="font-sans-tech px-4 py-3 rounded-2xl transition-colors" style={{ fontSize: 15, fontWeight: 500, color: T.body }}>
            🇲🇽 Español
          </a>
      ...
      <div className="relative z-20 grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr] gap-10 lg:gap-8 px-6 md:px-12 lg:px-16 pt-28 lg:pt-32 pb-16">
        ...
        <div>
          <div className="inline-flex items-center gap-3 font-mono uppercase" style={{
            ...
            <span style={{ color: T.body }}>continuous updates</span>
          </div>
          ...
          <h1 className="font-serif-display" style={{ margin: '32px 0 0', fontSize: 'clamp(56px, 9vw, 112px)', lineHeight: 0.94, letterSpacing: '-0.04em', fontWeight: 500, color: T.text }}>
            ...
            <span style={{ fontStyle: 'italic', fontWeight: 400,
              ...
              filter: `drop-shadow(0 0 40px ${T.cyan}55)` }}>{homeT.title2}</span>
          </h1>
          ...
          <p className="font-serif-display" style={{ margin: '32px 0 0', fontSize: 'clamp(17px,2.2vw,20px)', lineHeight: 1.45, color: T.body, maxWidth: 560 }}>
            Continuous intelligence on the deployment of AI across the Mexican state —
            <span style={{ color: T.text }}> synthesized, verified, and always on.</span>
          </p>
          ...
          <div className="mt-10 flex flex-wrap gap-3.5 items-center">
            ...
            <a href="/en/metodologia" className="font-sans-tech" style={{
          ...
          <div className="mt-12 flex flex-wrap gap-3">
            <MetricPill n={nAnuncios} l="Announcements" c={T.cyan} href="/en" />
            ...
            <MetricPill n={nProductos} l="Operating" c={T.blue} href="/en" />
            <MetricPill n={nIniciativas} l="Bills" c={T.green} href="/en/legislacion" />
            <MetricPill n={nCasos} l="Cases" c={T.violet} href="/en/casos-ia" />
          </div>
        </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            <button aria-label="Close subscription" onClick={() => formStatus !== 'loading' && setShowModal(false)} className="absolute top-4 right-4 text-gray-400 hover:text-gray-600" disabled={formStatus === 'loading'}>
            ...
            {formStatus === 'success' ? (
              <div className="text-center py-8">
                ...
                <h3 id="subscription-title" className="font-serif-display text-2xl text-gray-900 mb-2">Thank you!</h3>
              ...
              <>
                ...
                <form onSubmit={handleSubscribe} className="space-y-4">
                  <div>
                    <label htmlFor="subscription-name-en" className="block text-xs font-sans-tech font-medium text-gray-700 mb-1.5">Full name *</label>
                    <input id="subscription-name-en" name="nombre" type="text" autoComplete="name" value={formData.nombre} onChange={(e) => setFormData({ ...formData, nombre: e.target.value })} placeholder="Your full name" className="w-full px-4 py-3 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required maxLength={120} disabled={formStatus === 'loading'} />
                  </div>
                  <div>
                    <label htmlFor="subscription-email" className="block text-xs font-sans-tech font-medium text-gray-700 mb-1.5">Email address *</label>
                    <input id="subscription-email" name="email" type="email" autoComplete="email" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} placeholder="you@email.com" className="w-full px-4 py-3 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required maxLength={254} disabled={formStatus === 'loading'} />
                  </div>
                  <div>
                    <label htmlFor="subscription-phone-en" className="block text-xs font-sans-tech font-medium text-gray-700 mb-1.5">Phone number with country code *</label>
                    <input id="subscription-phone-en" name="telefono" type="tel" inputMode="tel" autoComplete="tel" value={formData.telefono} onChange={(e) => setFormData({ ...formData, telefono: e.target.value })} placeholder="+52 55 1234 5678" className="w-full px-4 py-3 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required maxLength={32} disabled={formStatus === 'loading'} />
                  </div>
                  <label className="flex items-start gap-3 text-xs leading-relaxed text-gray-600">
                    ...
                    />
                    <span>I agree to receive email updates from the Observatorio and can unsubscribe at any time.</span>
                  </label>
                  <label className="flex items-start gap-3 text-xs leading-relaxed text-gray-600">
                    <input
                      type="checkbox"
                      ...
                      disabled={formStatus === 'loading'}
                    />
                    <span>I agree to receive Observatorio alerts on WhatsApp and can opt out at any time.</span>
                  </label>
...
function IrisMark() {
  return (
    <svg width="24" height="24" viewBox="0 0 100 100" fill="none" aria-hidden="true">
      <defs><linearGradient id="heroIrisEn" x1="0" y1="0" x2="100" y2="100" gradientUnits="userSpaceOnUse"><stop stopColor="#3DE0FF" /><stop offset="1" stopColor="#4D7BFF" /></linearGradient></defs>
      <path d="M6 50 Q50 24 94 50 Q50 76 6 50 Z" stroke="#48556F" strokeWidth="4" />
      <circle cx="50" cy="50" r="21" stroke="url(#heroIrisEn)" strokeWidth="6" strokeDasharray="11 8" />
      <circle cx="50" cy="50" r="9" fill="url(#heroIrisEn)" />
    </svg>
  );
}
