...
// Twin en inglés de MetodologiaDAG.tsx — misma estructura, componentes y lógica;
// solo cambia el texto visible (labels, descripciones, fuentes). Sección
// Metodología — diseño "DAG Flow (Light)" elegido por Ricardo. Tema claro.
// Desktop (lg+): diagrama de flujo DAG animado (verticales → pipeline).
...
const TIERS: { key: string; color: string; label: string; desc: string }[] = [
  { key: 'oficial', color: C.green, label: 'Official source', desc: 'Verified against a government or judicial source: gob.mx, DOF, SCJN/SJF, congresses or public bodies.' },
  { key: 'documentada', color: C.cyan, label: 'Documented source', desc: 'Backed by a verifiable public source (e.g., a national newspaper), pending official corroboration.' },
  { key: 'sin_verificar', color: '#C77E12', label: 'No source cited', desc: 'Internal marker. We do not publish data without at least one citable, accessible source.' },
];

export default function MetodologiaDAGEn({ anuncios, iniciativas, casos }: Props) {
  const VERTICALS: Vertical[] = [
    {
      id: '01', code: 'ANNOUNCE', accent: C.cyan, title: 'Official Announcements', cat: 'Government · Communication', n: anuncios,
      desc: 'Government promises about AI projects: labs, platforms, investment.',
      sources: [['Morning press conferences', 'GOB'], ['Diario Oficial de la Federación (Official Gazette of the Federation)', 'DOF'], ['Agency press releases', 'SHCP']],
    },
    {
      id: '02', code: 'LEGIS', accent: C.green, title: 'AI Legislation', cat: 'Congress · Regulation', n: iniciativas,
      desc: 'Federal and state bills that regulate or mention artificial intelligence.',
      sources: [['Gaceta Parlamentaria (Parliamentary Gazette)', 'DIP'], ['Sistema de Información Legislativa (Legislative Information System)', 'SIL'], ['State congresses', '32 ENT']],
    },
    {
      id: '03', code: 'LITIG', accent: C.violet, title: 'Judicial Cases', cat: 'Judiciary · Litigation', n: casos,
      desc: 'Precedents where AI is the subject of litigation or a tool in the judicial process.',
      sources: [['Semanario Judicial de la Federación (Federal Judicial Weekly)', 'SJF'], ['SCJN (Supreme Court) portal', 'SCJN'], ['Collegiate Tribunals and the TFJA (Federal Administrative Justice Tribunal)', 'TFJA']],
    },
  ];
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 py-12 sm:py-16">
        ...
        <header className="text-center max-w-2xl mx-auto">
          <div className="inline-flex items-center gap-2.5 font-mono uppercase" style={{ fontSize: 11, letterSpacing: '0.26em', color: C.cyan, fontWeight: 600 }}>
            ...
            <span className="hidden sm:inline">03 verticals</span>
          </div>
        ...
        <div className="hidden lg:block mt-14 relative">
          ...
          <div className="absolute inset-0">
                ...
                <div key={v.id} style={{ position: 'absolute', left: 0, top, transform: 'translateY(-50%)', width: '24%' }}>
                  <div style={{ border: `1px solid ${v.accent}40`, background: C.panel, borderRadius: 12, padding: '13px 15px', display: 'flex', gap: 12, alignItems: 'center', boxShadow: `0 6px 20px -10px ${v.accent}55, 0 1px 0 rgba(255,255,255,0.7)` }}>
                    ...
                    <div style={{ minWidth: 0 }}>
                      ...
                      <div className="font-mono" style={{ fontSize: 9, letterSpacing: '0.14em', color: v.accent, marginTop: 3, fontWeight: 600 }}>{v.code} · {reg(v.n)} RECS</div>
                    </div>
                  </div>
                </div>
              );
            ...
            <div className="font-mono" style={{ position: 'absolute', left: `${440 / 1160 * 100}%`, top: '50%', transform: 'translate(-50%, -82px)', fontSize: 9, letterSpacing: '0.18em', color: C.faint, textTransform: 'uppercase', whiteSpace: 'nowrap' }}>▼ INPUT</div>
          </div>
        </div>
        ...
        <div className="lg:hidden mt-10 space-y-8">
                ...
                <div key={v.id} style={{ border: `1px solid ${v.accent}33`, background: C.panel, borderRadius: 16, padding: 18, boxShadow: `0 8px 24px -16px ${v.accent}66` }}>
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div>
                      ...
                      <div className="font-mono" style={{ fontSize: 9.5, letterSpacing: '0.14em', color: v.accent, marginTop: 2, fontWeight: 600 }}>{v.code} · {reg(v.n)} RECS</div>
                    </div>
                  </div>
          ...
          <div style={{ border: `1px solid ${C.line}`, background: C.panel, borderRadius: 16, padding: '20px 16px' }}>
            <div className="font-mono text-center" style={{ fontSize: 10, letterSpacing: '0.2em', color: C.faint, textTransform: 'uppercase', marginBottom: 16 }}>Verification pipeline</div>
        ...
        <div className="mt-14" style={{ borderTop: `1px solid ${C.line}`, paddingTop: 40 }}>
          <div className="text-center max-w-2xl mx-auto">
            ...
            <h3 className="font-serif-display" style={{ margin: '14px 0 0', fontWeight: 500, fontSize: 'clamp(24px,4vw,36px)', lineHeight: 1.05, letterSpacing: '-0.03em', color: C.ink }}>
              Sources and <em style={{ fontStyle: 'italic', fontWeight: 400, color: C.green }}>confidence level</em>
            </h3>
            <p className="font-serif-display" style={{ margin: '14px auto 0', maxWidth: 640, fontSize: 'clamp(15px,2vw,17px)', lineHeight: 1.55, color: C.body }}>
              Every record is published with its source cited. We prioritize official sources; national newspapers serve only as backup — <b style={{ color: C.ink, fontWeight: 500 }}>never an unofficial site as the sole source</b>.
            </p>
          </div>
