...
// mesLabel llega de Firestore en espanol (ver lib/agents.ts) y no tiene overlay
// propio -- se deriva el nombre del mes en ingles a partir de RecapData.mes
// (1-12) y solo si algo sale de rango se cae de vuelta al mesLabel original.
const MESES_EN = [
  'January', 'February', 'March', 'April', 'May', 'June',
  'July', 'August', 'September', 'October', 'November', 'December',
];

function mesLabelEn(recap: RecapData): string {
  return MESES_EN[(recap.mes || 0) - 1] || recap.mesLabel;
}

export default function RecapPageEn() {
    ...
    async function fetchRecaps() {
      try {
        const [response, overlay] = await Promise.all([
          fetch('/api/recap'),
          fetchOverlayEn('recaps'),
        ]);
        if (!response.ok) throw new Error('Error ' + response.status);
        const data = await response.json();
        if (data.recaps) {
          const traducidos = (data.recaps as RecapData[]).map((recap) =>
            aplicarOverlayDoc(recap, overlay, recap.id)
          );
          setRecaps(traducidos);
          if (traducidos.length > 0) {
            setSelectedRecap(traducidos[0]);
          }
        }
      } catch (err) {
        console.error('Error fetching recaps:', err);
        setError('The monthly recaps could not be loaded.');
      } finally {
        setLoading(false);
      }
    }
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center py-20">
        ...
        <p className="text-gray-500 font-sans-tech text-sm">Loading monthly recaps...</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-white">
      <main className="max-w-6xl mx-auto px-4 py-8 sm:py-12">
        <div className="mb-10">
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-amber-50 border border-amber-200/50 rounded-full mb-4">
            ...
            <span className="text-xs font-sans-tech text-amber-700 font-medium">Monthly Recap</span>
          </div>
          <h1 className="font-serif-display text-4xl sm:text-5xl md:text-6xl font-light text-gray-900 mb-3">
            Monthly <span className="italic text-blue-500">Summary</span>
          </h1>
          <p className="text-gray-600 font-sans-tech text-sm sm:text-base max-w-2xl">
            Each month, an AI-generated executive summary on the state of artificial intelligence
            in the Mexican government. Hard data, no frills.
          </p>
        </div>
        ...
        {recaps.length === 0 ? (
          <div className="text-center py-20 bg-gray-50 rounded-2xl border border-gray-200">
            ...
            <h3 className="font-sans-tech text-lg text-gray-700 mb-2">No recaps available</h3>
            <p className="text-gray-500 font-sans-tech text-sm max-w-md mx-auto">
              The first monthly recap will be generated on the 1st of next month.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            <div className="lg:col-span-1">
              <h3 className="font-sans-tech text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Editions</h3>
              <div className="space-y-2">
                {recaps.map((recap) => (
                  ...
                  >
                    <div className="font-medium text-sm capitalize">{mesLabelEn(recap)} {recap.anio}</div>
              ...
              <div className="lg:col-span-3">
                <div className="mb-8">
                  <div className="flex items-center gap-2 mb-3">
                    <span className="px-2.5 py-1 bg-blue-100 text-blue-700 rounded-full text-xs font-sans-tech font-medium capitalize">{mesLabelEn(selectedRecap)} {selectedRecap.anio}</span>
                    {selectedRecap.createdAt && (
                      <span className="text-xs text-gray-400 font-mono">{new Date(selectedRecap.createdAt).toLocaleDateString('en-US')}</span>
                    )}
                  </div>
                ...
                {selectedRecap.statsSnapshot && (
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
                    <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 text-center">
                      ...
                      <div className="text-[11px] font-sans-tech text-gray-500 uppercase tracking-wider">Announcements</div>
                    </div>
                    <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 text-center">
                      <div className="font-serif-display text-2xl text-emerald-600">{selectedRecap.statsSnapshot.operando}</div>
                      <div className="text-[11px] font-sans-tech text-emerald-600 uppercase tracking-wider">Operating</div>
                    </div>
                    <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 text-center">
                      ...
                      <div className="text-[11px] font-sans-tech text-blue-600 uppercase tracking-wider">In operation</div>
                    </div>
                    <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-center">
                      <div className="font-serif-display text-2xl text-amber-600">{selectedRecap.statsSnapshot.totalIniciativas || '—'}</div>
                      <div className="text-[11px] font-sans-tech text-amber-600 uppercase tracking-wider">Bills</div>
                    </div>
                  </div>
                )}
                {selectedRecap.datosClave && selectedRecap.datosClave.length > 0 && (
                  <div className="bg-gray-900 rounded-2xl p-6 mb-8">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <h3 className="font-sans-tech text-sm font-semibold text-white">Key Data</h3>
                    </div>
                ...
                {selectedRecap.veredicto && (
                  <div className="bg-blue-50 border border-blue-200 rounded-2xl p-6 mb-8">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <h3 className="font-sans-tech text-xs font-semibold text-blue-600 uppercase tracking-wider mb-2">This Month's Verdict</h3>
                ...
                {selectedRecap.fuentesConsultadas && selectedRecap.fuentesConsultadas.length > 0 && (
                  <div className="border-t border-gray-200 pt-6">
                    <h3 className="font-sans-tech text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Sources Consulted</h3>
                    <div className="space-y-2">
      ...
      <footer className="border-t border-gray-200 py-8 mt-12">
        <div className="max-w-6xl mx-auto px-4 text-center">
          <p className="text-xs text-gray-400 font-sans-tech">Generated by AI agents</p>
        </div>
      </footer>
    </div>
  );
