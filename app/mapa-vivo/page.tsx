...
// El mapa vivo — vitrina 3D del ecosistema: cada punto es un registro público
// real y cada línea una relación documentada. Los conteos del encabezado salen
// de las colecciones (OIA-009); los puntos, del /api/grafo (ISR ~5 min).

export const metadata: Metadata = {
  title: 'El mapa vivo — cada punto es un registro real',
  description:
    'La constelación del Observatorio IA México: anuncios, iniciativas, casos, temas y personas del ecosistema de IA en el Estado mexicano, conectados y con fuente.',
  alternates: { canonical: '/mapa-vivo', languages: { es: '/mapa-vivo', en: '/en/mapa-vivo' } },
};
  ...
  return (
    <div className="relative h-[100svh] min-h-[560px] w-full overflow-hidden" style={{ background: '#05070C', color: '#E7ECF7' }}>
      ...
      <div className="absolute right-5 top-5 md:right-10 md:top-8 z-10 text-right font-sans-tech text-[13px] leading-6 text-[#7886A2] pointer-events-none">
        <div><strong className="text-[#E7ECF7]">{d.puntos.length.toLocaleString('es-MX')}</strong> objetos públicos</div>
        <div><strong className="text-[#E7ECF7]">{d.enlaces.length}</strong> relaciones documentadas</div>
        <div><strong className="text-[#E7ECF7]">5</strong> entes del Estado y la sociedad</div>
      </div>
